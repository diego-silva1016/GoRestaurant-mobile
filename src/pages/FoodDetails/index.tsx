...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      try {
        ...
        const extrasFormatted = data.extras.map(item => ({
          ...item,
          quantity: 0,
        }));

        const isFavoriteFood = response.data.find(
  ...
  function handleDecrementExtra(id: number): void {
    setExtras(oldValues =>
      oldValues.map(extra => {
        if (id === extra.id && extra.quantity > 0)
          return {
            ...extra,
            quantity: extra.quantity - 1,
          };
  ...
  const cartTotal = useMemo(() => {
    const priceExtras = extras.reduce(function (accumulator, currentValue) {
      return accumulator + currentValue.quantity * currentValue.value;
    }, 0);
    return formatValue(foodQuantity * food.price + priceExtras);
